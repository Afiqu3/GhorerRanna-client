...
const LatestMeals = () => {
  ...
  useEffect(() => {
    const fetchMeals = async () => {
      try {
        setLoading(true);
        const response = await axiosSecure.get('/latest-meals');
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mt-10 text-center">
          <Link
            to={'/meals'}
            target="_parent"
            className="bg-secondary text-primary px-4 py-2 rounded-full font-semibold hover:bg-[#ffb73a] transition-all"
          >
            See All Meals
          </Link>
        </div>
